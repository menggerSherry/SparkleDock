...
#DSUB -q root.default
#DSUB -R cpu=4;gpu=4
#DSUB -oo scale8_%J.out					
#DSUB -eo scale8_%J.err		
...
mpirun -np 128 -hostfile ${JOB_ID}.nodefile -x PATH -x LD_LIBRARY_PATH -x USE_CUDA -x NUM_THREADS --mca plm_rsh_agent /opt/batch/agent/tools/dstart $dock_home/bin/lightdock -f setup.json -s $STEPS -l 1 
